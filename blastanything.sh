...
function blastanything () {
  ...
  if [ -f ${QUERY} ]; then
	...
	QUERYNAME=$(basename ${QUERY})
      	CALL_FUNC=echo $(${BLASTSUITE} -db "${BLASTDBDIR}" -query "${QUERY}" -out "${BLASTRESULTSDIR}/${QUERYNAME}.${BLASTSUITE}" -outfmt "6 qseqid sseqid length mismatch gapopen qstart qend sstart send evalue qcovhsp" -qcov_hsp_perc ${QCOV} -max_target_seqs 1)
