...
arquivo_fasta_modificado="$(echo $1 | sed -e 's/\.fasta$//')_modificado.fasta" # Nome de arquivo modificado

# Verifica se os arquivos existem e são legí­veis
if [ ! -f "$arquivo_fasta_original" ] || [ ! -r "$arquivo_fasta_original" ]; then
  echo "Erro: O arquivo FASTA original '$arquivo_fasta_original' não existe ou não pode ser lido."
  exit 1
fi

if [ ! -f "$arquivo_novos_labels" ] || [ ! -r "$arquivo_novos_labels" ]; then
  echo "Erro: O arquivo de novos labels '$arquivo_novos_labels' não existe ou não pode ser lido."
  exit 1
fi
